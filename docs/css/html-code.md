...
什么是语义化？就是用 __合理、正确的标签__ 来展示内容，比如 h1~h6 定义标题。
...
__Doctype__ 是一种 __标准通用标记语言__ 的文档类型声明，目的是告诉标准通用标记 __语言解析器__ 要使用什么样的文档类型定义（DTD）来解析文档。

__声明__ 是用来指示 web 浏览器关于页面使用哪个 HTML 版本进行编写的指令。 声明必须是 HTML 文档的第一行，位于 html 标签之前。

浏览器本身分为两种模式，一种是 __标准模式__，一种是 __怪异模式__，浏览器通过 Doctype 来区分这两种模式。
